...
async function run() {
  ...
  const siteId = "xyz_test_ui_" + Date.now();
  const siteRef = doc(db, 'dive_sites', siteId);
  
  console.log("Creating site...");
  await setDoc(siteRef, {
    userId: auth.currentUser.uid,
    name: "Test Update Site UI",
    lat: 1.123, lng: 2.123,
    // NO STATUS FIELD
  ...
  try {
    await updateDoc(siteRef, {
      name: "Updated Name UI",
      lat: 1.123, // Identical value
      lng: 2.123, // Identical value
      timestamp: serverTimestamp() // Only timestamp changes
    });
